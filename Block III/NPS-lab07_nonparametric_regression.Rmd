...
for (i in 1:10){
  l = lm(wage ~ poly(age,degree=degree))
}
```
...
attach(Prestige)
head(
       cut(income, breaks = c(min(income), 5000, 10000,max(income)))
       )
detach(Prestige)
```
...
```{r}
m_cut= lm(prestige ~ 1 + cut(income,breaks = c(-Inf,10000,max(income))), data=Prestige)
...
model_cut_disc=lm(prestige ~ 1 + income + income_cut_model + I(income>cutoff), data=Prestige)
...
```{r}
model_cut=lm(prestige ~ 1 + income + income_cut_model, data=Prestige)
