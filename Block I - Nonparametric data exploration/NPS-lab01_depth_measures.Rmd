...
$${M}_{h}D({F_n;\mathbf{x}}) = \frac{ 1 }{ 1 + {{(x - \hat{\mu}_F)} ^ {T}}{{\hat{\Sigma}}_F ^ {-1}}(x - \hat{\mu}_F) }$$
...
#devtools::install_local("~/Downloads/MDBED_1.0.0.tar.gz")   
library(MDBED)
...
-   some outliers may be wrongly flagged as genuine points (**masking**)
-   some good points may be wrongly flagged as outliers (**swamping**)
...
The _breakdown point_ measures the minimum data corruption required to bring estimates very far away from the correct values.

```{r}
fracs <- seq(0.1, .5, length.out=25)  # vector of contamination percentages
set.seed(41703192)
n_good <- 100
...
for (i in 1:length(fracs)){  # iterate across different values of contamination
  n=100
  # sample points
  frac <- fracs[i]
  n_good=ceiling(n*(1-frac))
...
Now we calculate (empirically) the measure of dependence of the estimator on the value of one of the points in the sample.
```{r}
sim.data <- rnorm(20) # univariate standard normal distribution sample
...
EIF1 <- numeric(L)
EIF2 <- numeric(L)
for (i in 1:length(grid.for.x)){   # iterate across different values
  sim.data.i <- sim.data
  sim.data.i[1] <- grid.for.x[i] # change the first value in the sample
